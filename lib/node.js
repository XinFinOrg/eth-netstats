...
Node.prototype.setBlock = function(block, history, latestCommittedBlockInfo, callback)
{
	if( !_.isUndefined(block) && !_.isUndefined(block.number) )
	{
		if ( !_.isEqual(history, this.history) || !_.isEqual(block, this.stats.block) )
		{
			...
			// Set the latestBlockInfo if it exist
			if(latestCommittedBlockInfo && latestCommittedBlockInfo.number && (this.stats.latestCommittedBlockInfo.hash != latestCommittedBlockInfo.hash)) {
			  this.stats.latestCommittedBlockInfo = latestCommittedBlockInfo;
			}
...
Node.prototype.getBlockStats = function()
{
	return {
		...
		propagationAvg: this.stats.propagationAvg,
		history: this.history,
		latestCommittedBlockInfo: this.stats.latestCommittedBlockInfo
	};
}
